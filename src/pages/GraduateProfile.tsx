    ...
    <div>
      <section className="relative bg-navbar border-b border-border overflow-hidden">
        ...
        <div className="relative container mx-auto px-4 lg:px-6 py-12">
          <Link to="/graduates" className="inline-flex items-center gap-2 text-sm text-foreground/70 hover:text-gold mb-6"><ArrowLeft className="h-4 w-4" /> Back to registry</Link>
          <div className="grid lg:grid-cols-[280px,1fr] gap-8 items-center">
            ...
            <div>
              <p className="text-gold font-heading text-xs tracking-[0.3em] uppercase mb-2">Certified Black Belt</p>
              <h1 className="font-display text-4xl md:text-6xl tracking-wider leading-[0.95]">{g.name}</h1>
              <p className="font-display text-2xl tracking-wider text-primary mt-2">{g.beltGrade}</p>
              <div className="grid sm:grid-cols-2 gap-x-8 gap-y-2 mt-6 text-sm">
      ...
      <section className="py-16">
        <div className="container mx-auto px-4 lg:px-6 grid lg:grid-cols-[1fr,320px] gap-10">
          <div>
            <h2 className="font-display text-3xl tracking-wider mb-6 text-gold">Graduation history</h2>
            <ol className="space-y-2 relative pl-10 border-l-2 border-border">
              {g.history.map((h) => (
                <li key={h.year} className="relative pb-6 last:pb-0">
                  ...
                  <div className="font-display text-2xl tracking-wider">{h.belt} <span className="text-gold">·</span> <span className="text-foreground/60">{h.year}</span></div>
                  <p className="text-foreground/70 mt-1 text-sm">Promoted by {h.promotedBy}</p>
                </li>
              ))}
            </ol>
          </div>
          ...
          <aside className="bg-card border border-gold/40 p-6 h-fit border-gold-hover">
            <div className="flex items-center gap-2 text-gold mb-3"><ShieldCheck className="h-5 w-5" /><span className="font-heading uppercase tracking-wider text-sm">Verified Certificate</span></div>
            <p className="text-xs text-muted-foreground mb-5">Scan to verify the authenticity of this graduation in the BJJLF registry.</p>
            <div className="aspect-square bg-white p-3 mb-4 grid place-items-center">
              <QrPlaceholder cert={g.certificateNo} />
            </div>
            <div className="text-xs space-y-1">
              <div className="flex justify-between"><span className="text-muted-foreground">Certificate №</span><span className="font-mono text-foreground">{g.certificateNo}</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">Issued</span><span className="text-foreground">{new Date(g.graduationDate).toLocaleDateString()}</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">Status</span><span className="text-gold">Active</span></div>
            </div>
            <Button variant="gold" className="w-full mt-5 font-heading uppercase tracking-wider">Verify Online</Button>
          </aside>
        </div>
      </section>
    </div>
...
function QrPlaceholder({ cert }: { cert: string }) {
  // Simple deterministic faux-QR pattern based on certificate number
  const grid = 12;
