    ...
    <div>
      <section className="relative overflow-hidden bg-navbar border-b border-border">
        ...
        <div className="relative container mx-auto px-4 lg:px-6 py-24 lg:py-32 max-w-4xl">
          <p className={cn(typo.heading.kicker, "mb-3")}>Our Federation</p>
          ...
          <p className={cn(typo.body.lg, "mt-6 max-w-2xl")}>
        ...
        <div className="container mx-auto px-4 lg:px-6 grid md:grid-cols-3 gap-6">
          {VALUES.map((v) => (
            <div key={v.title} className="bg-card border border-border p-8 border-gold-hover">
              ...
              <h3 className={cn(typo.heading.md, "mb-2 text-gold")}>{v.title}</h3>
              <p className={typo.body.md}>{v.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="py-20 bg-navbar/40 border-y border-border">
        <div className="container mx-auto px-4 lg:px-6 max-w-3xl">
          <p className={cn(typo.heading.kicker, "mb-3")}>History</p>
          <h2 className={cn(typo.heading.lg, "mb-10")}>Our journey</h2>
          <ol className="space-y-2 relative pl-10 border-l-2 border-border">
            {TIMELINE.map((t) => (
              <li key={t.year} className="relative pb-8 last:pb-0">
                ...
                <div className={cn(typo.heading.md, "text-gold")}>{t.year}</div>
                <p className={cn(typo.body.md, "mt-1 text-foreground/80")}>{t.event}</p>
              </li>
            ))}
          </ol>
        </div>
        ...
        <div className="container mx-auto px-4 lg:px-6 grid lg:grid-cols-2 gap-12 items-center">
          <div className="relative aspect-[4/5] overflow-hidden border border-gold/30">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6">
              <p className={cn(typo.heading.kicker, "mb-1")}>President & Founder</p>
              <h3 className={typo.heading.md}>Mestre Roberto Nogueira</h3>
            </div>
          </div>
          <div>
            <Award className="h-10 w-10 text-gold mb-4" />
            <h2 className={cn(typo.heading.lg, "mb-5")}>A life on the mat.</h2>
            <p className={cn(typo.body.md, "text-foreground/80 mb-4")}>A 7th-degree red-and-black belt with over 50 years of teaching, Mestre Roberto has trained world champions across four generations and personally graduated more than 80 black belts.</p>
            <p className={typo.body.md}>He founded BJJLF in 2008 with a clear mission: protect the art from dilution, certify only those who earn it, and ensure that no master, professor, or student is forgotten.</p>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 bg-navbar/40 border-t border-border">
        <div className="container mx-auto px-4 lg:px-6">
          <p className={cn(typo.heading.kicker, "mb-3")}>Leadership</p>
          <h2 className={cn(typo.heading.lg, "mb-10")}>Board of Directors</h2>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-5">
            {BOARD.map((m) => (
              <div key={m.name} className="text-center group">
                ...
                <h4 className={cn(typo.heading.sm, "mt-3 text-sm")}>{m.name}</h4>
                <p className={cn(typo.heading.kicker, "mt-1")}>{m.role}</p>
              </div>
            ))}
          </div>
        </div>
