 ...
 *
 * Allowed in: `src/lib/typography.ts` (the source of truth) and `.css` files.
 */
...
const rule = {
  meta: {
    ...
    messages: {
      ...
      inlineFontWeight:
        'Avoid inline `fontWeight` styles for typography. Use the `typo` tokens from "@/lib/typography" which already encode weight.',
      cssFontFamily:
        'Avoid raw `font-family:` declarations in component source. Define them in styles.css or use the `typo` tokens from "@/lib/typography".',
    },
  },
      ...
      JSXAttribute(node) {
        ...
        if (v.type === "Literal") {
          ...
          if (expr.type === "Literal") {
            ...
            for (const q of expr.quasis) checkClassString(context, node, q.value.cooked);
